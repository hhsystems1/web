...
function updateBranding() {
    ...
    function hideNavbarText() {
        ...
        navbarSpans.forEach(span => {
            if (span.textContent.includes('Helping Hands Web') || 
                ...
                // Only hide if it's not our custom brand
                if (!span.classList.contains('desktop-brand')) {
    ...
    // Add "Helping Hands Systems" to floating navbar on desktop/tablet (prevent duplicates)
    function addDesktopBranding() {
        if (window.innerWidth >= 768) {
            ...
            if (floatingLogo && !floatingLogo.querySelector('.desktop-brand')) {
                // Remove any existing brand text first
                const existingBrands = floatingLogo.querySelectorAll('.desktop-brand');
                existingBrands.forEach(brand => brand.remove());
            ...
            // Remove desktop brand on mobile
            const existingBrands = document.querySelectorAll('.desktop-brand');
            existingBrands.forEach(brand => brand.remove());
        }
    }
    
    // Run hiding first, then branding
    hideNavbarText();
    updateFooterBranding();
    
    // Add desktop branding after a tiny delay to ensure text is hidden first
    setTimeout(() => {
        addDesktopBranding();
    }, 50);
}
